...
def print_model_initialized(config):
    print("=" * 50)
...
def load_util_prompt(util_prompt_name):
    util_prompt_path = f"data/util_prompts/{util_prompt_name}.txt"
    with open(util_prompt_path, 'r') as f:
        return f.read().strip()


def replace_slash(s, replacement="--"):
...
def init_ray(num_cpus=32):
    ...
    # Needed to avoid import error: https://github.com/vllm-project/vllm/pull/871
    init_hf_modules()
...
def save_open_instruct_training_data(output_path, prompts, completions, data_ids=None, dataset_name=""):
    ...
    if data_ids is None:
        ...
        if len(data_ids) != len(prompts):
            raise ValueError(
                "Data IDs must have the same length as prompts and completions.")
        else:
            if dataset_name != "":
                data_ids = [f"{x}--{dataset_name}" for x in data_ids]
